...
take_profits = [1000, 2000, 3000, 4000, 5000, 10000]

# --- Function to simulate with take-profit ---
...
# --- Function to simulate without take-profit ---
...
# --- Risk metrics ---
def sharpe_ratio(portfolio_paths):
    daily_returns = (portfolio_paths[:, 1:] / portfolio_paths[:, :-1]) - 1
    mean_daily = np.mean(daily_returns)
    std_daily = np.std(daily_returns)
    return (mean_daily / std_daily) * np.sqrt(days_per_year) if std_daily != 0 else np.nan
...
sharpe_with_tp = sharpe_ratio(paths_with_tp)
sharpe_no_tp = sharpe_ratio(paths_no_tp)

max_dd_with_tp = np.mean([max_drawdown(path) for path in paths_with_tp]) * 100
max_dd_no_tp = np.mean([max_drawdown(path) for path in paths_no_tp]) * 100

# --- Print results ---
print(f"--- Results over {num_years} years ---")
print(f"With Take-Profit: Avg Final = {np.mean(final_with_tp):,.0f}, Target Hit Prob = {prob_with_tp:.2f}%, Sharpe = {sharpe_with_tp:.2f}, Avg Max DD = {max_dd_with_tp:.2f}%")
print(f"No Take-Profit:   Avg Final = {np.mean(final_no_tp):,.0f}, Target Hit Prob = {prob_no_tp:.2f}%, Sharpe = {sharpe_no_tp:.2f}, Avg Max DD = {max_dd_no_tp:.2f}%")
...
values_range = np.linspace(min(final_with_tp.min(), final_no_tp.min()),
                           max(final_with_tp.max(), final_no_tp.max()),
                           500)
cum_prob_with_tp = [np.mean(final_with_tp >= v) * 100 for v in values_range]
cum_prob_no_tp = [np.mean(final_no_tp >= v) * 100 for v in values_range]

plt.figure(figsize=(12, 6))
plt.plot(values_range, cum_prob_with_tp, color='blue', linewidth=2, label='With Take-Profit')
plt.plot(values_range, cum_prob_no_tp, color='orange', linewidth=2, label='No Take-Profit')
plt.axvline(target_portfolio, color='red', linestyle='--', label='Target')
plt.title('Cumulative Probability of Ending Above Portfolio Value')
plt.xlabel('Final Portfolio Value')
